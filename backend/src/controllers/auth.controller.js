...
const getClientBaseUrl = () => {
  const raw = process.env.CLIENT_URL || process.env.CLIENT_URLS?.split(",")?.[0] || "http://localhost:5173";
  return raw.replace(/\/+$/, "");
};
...
const sendPasswordResetEmail = async ({ user, resetToken }) => {
  ...
  await sendEmail({
    to: user.email,
    subject: "Reset your LinePe password",
    text: `Reset your password using this link: ${resetUrl}. It expires in 15 minutes.`,
    html: `
      <div style="font-family:Arial,sans-serif;line-height:1.5;color:#111">
        <h2>Reset your LinePe password</h2>
...
const sendEmailVerificationLink = async ({ user, verificationToken }) => {
  const verifyUrl = `${getClientBaseUrl()}/api/auth/verify-email/${verificationToken}`;
  await sendEmail({
    to: user.email,
    subject: "Verify your LinePe email",
    text: `Verify your email using this link: ${verifyUrl}. It expires in 24 hours.`,
    html: `
      <div style="font-family:Arial,sans-serif;line-height:1.5;color:#111">
...
export const resetPassword = async (req, res, next) => {
  try {
    const token = sanitizePlainText(req.params.token, { maxLength: 200 });
...
export const verifyEmail = async (req, res, next) => {
  try {
    ...
    const redirectUrl = `${getClientBaseUrl()}/profile?verified=1`;
...
export const updateProfile = async (req, res, next) => {
  try {
    const { profilePic, username, phoneNumber, currentPassword, newPassword } = req.body;
    const userId = req.user._id;
    const nextCurrentPassword = typeof currentPassword === "string" ? currentPassword : "";
    const nextNewPassword = typeof newPassword === "string" ? newPassword : "";
    ...
    const wantsPasswordChange = Boolean(nextCurrentPassword || nextNewPassword);

    if (!profilePic && typeof username !== "string" && !hasPhoneNumber && !wantsPasswordChange) {
      return res.status(400).json({ message: "Profile pic, username, phone number, or password is required" });
    }
    ...
    if (wantsPasswordChange) {
      ...
      const matches = await bcrypt.compare(nextCurrentPassword, user.password);
      if (!matches) {
        return res.status(400).json({ message: "Current password is incorrect" });
      }

      const salt = await bcrypt.genSalt(10);
      updates.password = await bcrypt.hash(nextNewPassword, salt);
    }
